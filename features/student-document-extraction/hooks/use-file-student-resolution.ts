...
export function useFileStudentResolution({ currentFileKeys, fileExtractions, students }: Params) {
  ...
  }, [fileMatchPlans, currentFileKeys, currentFileKeySet]);
      ...
      plan.pendingAi?.tasks.forEach((task) => {
        ...
        void matchStudentName(task.detectedNames, task.candidates)
          .then((result) => {
            ...
            if (
              !task.candidates.some((candidate) => candidate.rowIndex === matchedRowIndex) ||
            ...
            setFileStudentMatches((previous) => {
              ...
              return {
                ...previous,
                [fileKey]: createFileStudentMatch(
                  [...baseRows, ...previousRows, matchedRowIndex],
                  source,
                ),
              };
            });
  ...
  const resolveFileStudent = (fileKey: string, taskKey: string, rowIndex: number) => {
    if (!currentFileKeySet.has(fileKey)) {
      return;
    }
  ...
  const ignoreFileStudent = (fileKey: string, taskKey: string) => {
    if (!currentFileKeySet.has(fileKey)) {
      return;
    }
