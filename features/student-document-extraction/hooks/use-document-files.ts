...
export function useDocumentFiles(): UseDocumentFilesReturn {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((previousFiles) => uniqueFiles([...previousFiles, ...newFiles]));
  ...
  const replaceFiles = (nextFiles: File[]) => {
    setFiles(uniqueFiles(nextFiles));
  };

  return { files, handleFileChange, handleRemoveFile, clearFiles, replaceFiles };
}
