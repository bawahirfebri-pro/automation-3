...
export function getStudentFileMembership({
  ...
  const rawRowsByFile = Object.fromEntries(
    currentFileKeys.map((fileKey) => [fileKey, fileStudentMatches[fileKey]?.rowIndexes ?? []]),
  ...
  const scopedRowsByFile = Object.fromEntries(
    currentFileKeys.map((fileKey) => {
      ...
      return [fileKey, scopedRows?.length ? scopedRows : (rawRowsByFile[fileKey] ?? [])];
    }),
  ) as Record<string, number[]>;

  return { rawRowsByFile, scopedRowsByFile };
}
