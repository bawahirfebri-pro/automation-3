...
export function getSessionReadiness({
  ...
  const currentKkFileKeys = currentFileKeys.filter((fileKey) => Boolean(fileExtractions[fileKey]?.kk));
  const currentAktaFileKeys = currentFileKeys.filter((fileKey) => Boolean(fileExtractions[fileKey]?.akta));
  const sessionStudentRowIndexes = [...new Set(currentFileKeys.flatMap((fileKey) => scopedRowsByFile[fileKey] ?? []))];
  const kkStudentRowIndexes = [...new Set(currentKkFileKeys.flatMap((fileKey) => scopedRowsByFile[fileKey] ?? []))];
    ...
    if (currentKkFileKeys.length > 0) {
      if (kkStudentRowIndexes.length > 0) {
        ...
        if (foreignAktaRows.length > 0) {
          ...
          sessionConflict = names.length > 0
  ...
  const unresolvedFileKeys = currentFileKeys.filter(
    (fileKey) => Boolean(fileExtractions[fileKey]) && (fileStudentMatches[fileKey]?.rowIndexes.length ?? 0) === 0
  );

  const duplicates: string[] = [];

  for (const student of sessionStudents) {
    const kkCount = currentKkFileKeys.filter(
      (fileKey) => (scopedRowsByFile[fileKey] ?? []).includes(student.rowIndex)
    ).length;

    const aktaCount = currentAktaFileKeys.filter(
      (fileKey) => (scopedRowsByFile[fileKey] ?? []).includes(student.rowIndex)
    ).length;
  ...
  const duplicateDocumentMsg = duplicates.length === 0
    ? ""
