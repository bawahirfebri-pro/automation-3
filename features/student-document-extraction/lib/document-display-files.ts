...
export function buildDocumentDisplayFiles({
  ...
  return files.flatMap((file): DocumentDisplayFile[] => {
    ...
    const documentType = extractedDocumentType === "unknown" ? null : extractedDocumentType ?? null;
    ...
    if (!documentType || rowIndexes.length === 0) {
      ...
      const canonicalDocumentType = documentType === "both" ? extraction?.akta ? "akta" : extraction?.kk ? "kk" : null : documentType;
      const displayName = extractedName && canonicalDocumentType ? createCanonicalFileName(extractedName, canonicalDocumentType) : file.name;
      return [{
        file,
        fileKey,
        outputKey: fileKey,
    ...
    return matchedStudents.map((student): DocumentDisplayFile => {
      const canonicalDocumentType = documentType === "both"
        ? fileExtractions[fileKey]?.akta ? "akta" : "kk"
        : documentType;
