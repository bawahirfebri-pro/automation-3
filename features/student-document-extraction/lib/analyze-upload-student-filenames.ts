...
export function analyzeUploadStudentFilenames(selectedFiles: File[], students: StudentRecord[]) {
  ...
  const filenameIssueEntries = selectedFiles.flatMap((file) => {
    ...
    const detectedName = match[1].replace(/[_-]+/g, " ").trim();
    ...
    return [[fileKey, { status: "not-found", detectedName }] as const];
  });
  ...
  const immediateNamedMatches = selectedFiles
    ...
    .filter((item): item is ImmediateNamedMatch => item.rowIndex !== null);

  const immediateNamedRows = [...new Set(immediateNamedMatches.map((item) => item.rowIndex))];

  const notFoundFileKeys = new Set(filenameIssueEntries.map(([fileKey]) => fileKey));

  const allSelectedNotFound =
    selectedFiles.length > 0 && notFoundFileKeys.size === selectedFiles.length;

  return {
    selectedKeys,
    filenameIssueEntries: filenameIssueEntries as readonly (readonly [
      string,
      FilenameMatchIssue,
    ])[],
