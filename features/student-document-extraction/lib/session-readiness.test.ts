...
function createBaseParams(
  overrides: Partial<SessionReadinessParams> = {}
  ...
  return {
    ...
    failedFileKeys: [],
    students: [
      createStudent(10, "Budi Santoso"),
      createStudent(20, "Siti Aminah"),
    ],
...
describe("getSessionReadiness", () => {
  ...
  it("mendeteksi resolution yang belum complete", () => {
    ...
    expect(
      result.unresolvedResolutionFileKeys
    ).toEqual(["file-a"]);
  ...
  it("mendeteksi Akta milik siswa di luar KK", () => {
    const result = getSessionReadiness(
      createBaseParams({
        ...
        fileExtractions: {
          ...
          "file-akta": createExtraction({
            type: "akta",
            akta: {} as never,
          }),
        },
        fileStudentMatches: {
          "file-kk": createMatch([10]),
          "file-akta": createMatch([20]),
        },
        fileResolutionComplete: {
          "file-kk": true,
          "file-akta": true,
    ...
    expect(result.sessionConflict).toContain(
      "Siti Aminah"
    );
    expect(result.sessionReady).toBe(false);
  });
  ...
  it("mendeteksi multi-student session tanpa KK sebagai conflict", () => {
    const result = getSessionReadiness(
      createBaseParams({
        ...
        fileExtractions: {
          "akta-a": createExtraction({
            type: "akta",
            akta: {} as never,
          }),
          "akta-b": createExtraction({
            type: "akta",
            akta: {} as never,
          }),
        },
        fileStudentMatches: {
          "akta-a": createMatch([10]),
          "akta-b": createMatch([20]),
        },
        fileResolutionComplete: {
          "akta-a": true,
          "akta-b": true,
  ...
  it("mendeteksi duplicate KK untuk student yang sama", () => {
    const result = getSessionReadiness(
      createBaseParams({
        ...
        fileExtractions: {
          "kk-a": createExtraction({
            type: "kk",
            kk: {} as never,
          }),
          "kk-b": createExtraction({
            type: "kk",
            kk: {} as never,
          }),
        },
    ...
    expect(result.duplicateDocumentMsg).toContain(
      "Budi Santoso"
    );
    expect(result.duplicateDocumentMsg).toContain(
      "KK"
