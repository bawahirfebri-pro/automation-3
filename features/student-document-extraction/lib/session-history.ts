...
export function buildSessionHistoryItems({
  ...
  return sessionStudents.flatMap((student) => {
    ...
    const kkExtraction =
      studentExtractions.find((item) => item.kk) ?? null;

    const aktaExtraction =
      studentExtractions.find((item) => item.akta) ?? null;
    ...
    const baseline =
      studentDetailBaseline?.rowIndex === student.rowIndex
        ? studentDetailBaseline
        : null;
