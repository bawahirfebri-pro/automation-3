...
interface UploadSectionProps {
  ...
  onRemoveFile: (fileKey: string, studentRowIndex: number | null) => void;
...
export default function UploadSection({
  ...
  const isBusy = isPreprocessing || isExtracting;

  const sortedDisplayFiles = useMemo(() => {
    return [...displayFiles]
      .map((item, originalIndex) => ({ item, originalIndex }))
      .sort((a, b) => {
        const aRenamed = a.item.renamed;

        const bRenamed = b.item.renamed;
         ...
         */
        if (aRenamed && bRenamed) {
          const nameA = (a.item.studentName || a.item.displayName || "").trim();

          const nameB = (b.item.studentName || b.item.displayName || "").trim();
  ...
  const getFileStatus = (displayFile: DocumentDisplayFile): FileStatus => {
    ...
    if (conflictMsg && (fileMatch?.rowIndexes.length ?? 0) > 0) {
      return { label: "Konflik", className: "bg-red-50 text-red-600" };
    }

    if (isAiMatching) {
      return { label: "Mencocokkan", className: "bg-violet-50 text-violet-700" };
    }

    if (!isProcessed && (isExtracting || hasPendingFiles)) {
      return { label: "Memproses", className: "bg-blue-50 text-blue-600" };
    }

    const filenameIssue = filenameMatchIssues[fileKey];

    if (filenameIssue?.status === "not-found") {
      return { label: "Tidak terdaftar", className: "bg-red-50 text-red-600" };
    }

    if (unresolvedTasks > 0) {
      return { label: "Perlu dipilih", className: "bg-amber-50 text-amber-700" };
    }
    ...
    if ((fileMatch?.rowIndexes.length ?? 0) > 0 && isSupportedDocument && isProcessed) {
      return { label: "Berhasil", className: "bg-emerald-50 text-emerald-700" };
    }

    if (isProcessed && !isSupportedDocument) {
      return { label: "Bukan KK/Akta", className: "bg-gray-100 text-gray-500" };
    }

    if (isProcessed) {
      return { label: "Mengidentifikasi", className: "bg-gray-100 text-gray-600" };
    }

    return { label: "Siap", className: "bg-gray-100 text-gray-500" };
  };
  ...
  const downloadFileFallback = (displayFile: DocumentDisplayFile) => {
  ...
  const handleSaveRenamedFile = async (displayFile: DocumentDisplayFile) => {
    if (!displayFile.renamed || hasBlockingIssue) {
      return;
    }
    ...
    if (!pickerWindow.showSaveFilePicker) {
      downloadFileFallback(displayFile);
    ...
    try {
      const handle = await pickerWindow.showSaveFilePicker({
        suggestedName: displayFile.displayName,
      ...
      await writable.write(displayFile.file);
  ...
  const handleSaveAllRenamedFiles = async () => {
    if (hasBlockingIssue || renamedFiles.length === 0) {
      return;
    }
    ...
    if (!pickerWindow.showDirectoryPicker) {
      renamedFiles.forEach((displayFile, index) => {
        window.setTimeout(() => downloadFileFallback(displayFile), index * 150);
      });
    ...
    try {
      ...
      for (const displayFile of renamedFiles) {
        const fileHandle = await directoryHandle.getFileHandle(displayFile.displayName, {
          create: true,
        });
      ...
      console.error("[SAVE ALL PDF]", error);
    }
  };
  ...
  return (
    <div className="flex h-[516px] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white">
      ...
      <div className="flex min-h-[73px] items-center justify-between gap-4 border-b border-gray-100 px-5 py-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-semibold text-gray-900">Tambahkan Dokumen</h2>
            <p className="text-xs text-gray-500">Upload KK dan Akta Kelahiran dalam format PDF</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col p-5">
              ...
              <div className="mb-3 flex shrink-0 items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-[10px] text-gray-400">
                    Pilih PDF baru untuk dimasukkan ke antrean.
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  disabled={isBusy}
                  className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-[10px] font-medium text-gray-600 transition hover:bg-gray-50 hover:text-gray-800 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <svg
                    viewBox="0 0 20 20"
                    fill="none"
              ...
              {isBusy ? (
                <>
                  ...
                  <span className="text-sm font-semibold text-gray-700">
                    {isPreprocessing ? "Sedang menyiapkan dokumen" : "Sedang memproses dokumen"}
                  </span>
        ...
        {queueVisible && (
          <div
            className={`${compactQueueMode ? "flex min-h-0 flex-1 flex-col" : "mt-5 shrink-0"} overflow-hidden rounded-xl border border-gray-100 bg-gray-50/50`}
          >
            <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3">
              <div>
                <div className="flex items-center gap-2">
                  <p className="text-xs font-semibold text-gray-700">Antrean Dokumen</p>
                ...
                <p className="mt-0.5 text-[11px] text-gray-400">
                  ...
                  {displayFiles.length !== files.length ? ` · ${displayFiles.length} output` : ""}
                </p>
              </div>
              ...
              <div className="flex shrink-0 items-center gap-2">
                {compactQueueMode && (
                  <button
                    ...
                    disabled={isBusy}
                    className="inline-flex h-7 items-center gap-1.5 rounded-md border border-blue-100 bg-blue-50 px-2.5 text-[10px] font-medium text-blue-600 transition hover:bg-blue-100 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <svg
                      viewBox="0 0 20 20"
                      fill="none"
            ...
            <ul
              className={`${compactQueueMode ? "min-h-0 flex-1" : "max-h-[194px]"} divide-y divide-gray-100 overflow-y-auto`}
            >
              {sortedDisplayFiles.map((displayFile) => {
                ...
                const showManualTasks = firstOutputKeyByFile.get(fileKey) === displayFile.outputKey;

                const unresolvedTasks = unresolvedTasksByFile[fileKey] ?? [];
                  ...
                  >
                    <div className="flex min-w-0 flex-1 items-start gap-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex min-w-0 items-center gap-1.5">
                          ...
                          {displayFile.virtual && displayFile.documentType === "kk" && (
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-1.5">
                          ...
                          {matchMethod && !conflictMsg && (
                            <span className="text-[10px] text-gray-400">{matchMethod}</span>
                          )}
                        </div>
                                    ...
                                    {!isOpen ? (
                                      ...
                                      <div className="mt-2 w-[240px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                                        ...
                                        <div className="max-h-[140px] overflow-y-auto p-1">
                                          {options.map((student) => {
                                            const alreadyClaimed = claimedRows.includes(
                                              student.rowIndex,
                                            );
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      {displayFile.renamed && !hasBlockingIssue && (
                        <button
                          type="button"
                          onClick={() => handleSaveRenamedFile(displayFile)}
                          disabled={isBusy}
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          onRemoveFile(displayFile.fileKey, displayFile.studentRowIndex)
                        }
                        disabled={isBusy}
          ...
          <div className="mt-4 shrink-0 rounded-xl border border-red-100 bg-red-50 px-4 py-3">
            <div className="flex items-start justify-between gap-4">
              <div className="flex min-w-0 gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-medium text-red-700">Dokumen tidak sesuai</p>
                  <p className="mt-0.5 text-[11px] leading-4 text-red-600">{conflictMsg}</p>
                </div>
              </div>
        ...
        {duplicateMsg && !conflictMsg && (
          <div className="mt-4 shrink-0 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3">
            <div className="flex gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-amber-800">Dokumen ganda</p>
                <p className="mt-0.5 text-[11px] leading-4 text-amber-700">{duplicateMsg}</p>
              </div>
            </div>
          </div>
        )}
