...
export function getFileResolutionState({
  ...
  const fileResolutionComplete = Object.fromEntries(
    currentFileKeys.map((fileKey) => {
      const plan = fileMatchPlans[fileKey];
      const currentMatch = fileStudentMatches[fileKey];
      const trustedFilenameMatch =
        currentMatch?.source === "exact" && currentMatch.rowIndexes.length > 0;
      ...
      return [
        fileKey,
        !aiMatchingFileKeys.includes(fileKey) && accounted >= plan.resolution.totalCandidates,
      ];
    }),
