...
function createAkta(): AktaResult {
  return { no_akta_kelahiran: "AKTA-001", nama_anak: "Budi Santoso" } as AktaResult;
}
...
describe("buildSessionSavePayload", () => {
  it("mengembalikan null jika student tidak termasuk session", () => {
    const result = buildSessionSavePayload({
      ...
      fileExtractions: { "file-a": createExtraction({ type: "kk", kk: createKk() }) },
    });
  ...
  it("mengembalikan null jika student tidak termasuk SCOPED file", () => {
    const result = buildSessionSavePayload({
      student: createStudent({ rowIndex: 20 }),
      sessionStudentRowIndexes: [20],
      currentFileKeys: ["shared-kk"],
      scopedRowsByFile: { "shared-kk": [10] },
      fileExtractions: { "shared-kk": createExtraction({ type: "kk", kk: createKk() }) },
    });
  ...
  it("membuat payload KK dari file yang termasuk SCOPED student", () => {
    ...
    const result = buildSessionSavePayload({
      ...
      scopedRowsByFile: { "file-kk": [10] },
      fileExtractions: { "file-kk": createExtraction({ type: "kk", kk }) },
    });
  ...
  it("tidak memasukkan KK jika data KK student sudah lengkap", () => {
    ...
    const result = buildSessionSavePayload({
      student: createStudent({ kkComplete: true }),
      sessionStudentRowIndexes: [10],
      currentFileKeys: ["file-both"],
      scopedRowsByFile: { "file-both": [10] },
      fileExtractions: { "file-both": createExtraction({ type: "both", kk: createKk(), akta }) },
    });
  ...
  it("tidak memasukkan Akta jika data Akta student sudah lengkap", () => {
    ...
    const result = buildSessionSavePayload({
      student: createStudent({ aktaComplete: true }),
      sessionStudentRowIndexes: [10],
      currentFileKeys: ["file-both"],
      scopedRowsByFile: { "file-both": [10] },
      fileExtractions: { "file-both": createExtraction({ type: "both", kk, akta: createAkta() }) },
    });
  ...
  it("mengembalikan null jika KK dan Akta sudah lengkap", () => {
    const result = buildSessionSavePayload({
      student: createStudent({ kkComplete: true, aktaComplete: true }),
      sessionStudentRowIndexes: [10],
      currentFileKeys: ["file-both"],
      scopedRowsByFile: { "file-both": [10] },
      fileExtractions: {
        "file-both": createExtraction({ type: "both", kk: createKk(), akta: createAkta() }),
      },
    });
