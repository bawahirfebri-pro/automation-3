...
type PendingUploadStatus = "failed" | "matching" | "processing" | "unresolved" | "ready";

interface PendingUploadStatusParams {
  hasFailed: boolean;
...
function getPendingUploadStatus({
  hasFailed,
  isAiMatching,
  isProcessing,
  hasUnresolvedTasks,
  hasStudent,
}: PendingUploadStatusParams): PendingUploadStatus {
  if (hasFailed) return "failed";
  if (isAiMatching) return "matching";
  if (isProcessing) return "processing";
  if (hasUnresolvedTasks || !hasStudent) return "unresolved";
...
function getPendingUploadStatusLabel(status: PendingUploadStatus, detectedName?: string): string {
  if (status === "failed") return "Gagal diproses";
  if (status === "matching") return "Mencocokkan murid...";
  if (status === "processing") return "Memproses dokumen...";
  if (status === "ready") return "Siap disimpan";

  return detectedName ? `${detectedName} belum ditemukan` : "Perlu memilih murid";
}

function getPendingUploadStatusClass(status: PendingUploadStatus): string {
  if (status === "failed") return "text-red-500";
  if (status === "ready") return "text-emerald-600";
  if (status === "unresolved") return "text-amber-600";
...
export default function PendingUploadTray({
  ...
  const items = useMemo(() => {
    const grouped = new Map<string, PendingUploadItem>();
      ...
      <div className="min-h-0 overflow-y-auto overscroll-contain px-1.5 pb-1.5">
        <ul className="space-y-0.5">
          {items.map((item) => {
            ...
            const status = getPendingUploadStatus({
              hasFailed,
              isAiMatching,
              isProcessing,
              hasUnresolvedTasks: unresolvedTasks.length > 0,
              hasStudent: item.studentRowIndex !== null,
            });
                ...
                <div className="rounded-md px-2.5 py-2">
                  <div className="flex min-w-0 items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p
                        className={`mt-0.5 truncate text-[9px] ${getPendingUploadStatusClass(
                          status,
                        )}`}
                      >
                        {getPendingUploadStatusLabel(status, filenameIssue?.detectedName)}
                      </p>
                    </div>
