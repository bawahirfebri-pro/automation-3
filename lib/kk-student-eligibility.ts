...
export function getKkStudentEligibility(
  ...
  const adultRelation =
    ADULT_RELATION_KEYWORDS.some(
      (keyword) =>
        relation === keyword ||
        relation.includes(keyword)
    );
...
export function getEligibleKkStudents(
  ...
  return anggotaKeluarga.filter((anggota) => {
    ...
    return getKkStudentEligibility(
      anggota,
      referenceDate
    ).eligible;
  });
