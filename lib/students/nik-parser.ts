...
export function parseDataFromNik(nikString: string | null | undefined): NikResult {
  const nik = String(nikString ?? "").replace(/\D/g, "");

  if (nik.length !== 16) {
    return { jenis_kelamin: "-", tanggal_lahir: "-" };
  }
   ...
   */
  const fullYear = tahun <= 30 ? 2000 + tahun : 1900 + tahun;

  const date = new Date(fullYear, bulan - 1, tanggal);

  const isValidDate =
    date.getFullYear() === fullYear && date.getMonth() === bulan - 1 && date.getDate() === tanggal;
  ...
  const tanggal_lahir =
    `${String(tanggal).padStart(2, "0")}-` + `${String(bulan).padStart(2, "0")}-` + `${fullYear}`;
