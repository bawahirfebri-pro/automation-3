const STUDENT_DOCUMENT_EXTRACTION_PATH =
  "/students/document-extraction";

export function getNikFromPathname(
  pathname: string
): string {
  return (
    pathname.match(
      /^\/students\/document-extraction\/(\d{16})\/?$/
    )?.[1] ??
    pathname.match(
      /^\/(\d{16})\/?$/
    )?.[1] ??
