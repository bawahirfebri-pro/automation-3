...
export function mapRowToStudentDetail(
  targetRow: GoogleSpreadsheetRow
): StudentDetail {
  const noKk = getSheetValue(
    targetRow,
    "No. Kartu Keluarga"
  );

  const noAkta = getSheetValue(
    targetRow,
    "No. Akta Kelahiran"
  );

  const anggotaKeluarga:
    KkResult["anggota_keluarga"] = [];

  for (let i = 1; i <= 10; i += 1) {
    const nama = getSheetValue(
      targetRow,
      `Nama Anggota ${i}`
    );

    const nik = getSheetValue(
      targetRow,
      `NIK Anggota ${i}`
    );
    ...
    anggotaKeluarga.push({
      ...
      tempat_lahir: getSheetValue(
        targetRow,
        `Tempat Lahir Anggota ${i}`
      ),
      tanggal_lahir: getSheetValue(
        targetRow,
      ...
      golongan_darah: getSheetValue(
        targetRow,
        `Golongan Darah Anggota ${i}`
      ),
      pendidikan: getSheetValue(
        targetRow,
        `Pendidikan Anggota ${i}`
      ),
      jenis_pekerjaan: getSheetValue(
        targetRow,
        `Pekerjaan Anggota ${i}`
      ),
      nama_ayah: getSheetValue(
        targetRow,
        `Nama Ayah dari Anggota ${i}`
      ),
      nama_ibu: getSheetValue(
        targetRow,
  ...
  const kk: KkResult | null = noKk
    ? {
        ...
        kelurahan: getSheetValue(
          targetRow,
          "Desa/Kelurahan"
        ),
        kecamatan: getSheetValue(
          targetRow,
          "Kecamatan"
        ),
        kabupaten_kota: getSheetValue(
          targetRow,
          "Kabupaten/Kota"
        ),
        provinsi: getSheetValue(
          targetRow,
          "Provinsi"
        ),
        kode_pos: getSheetValue(
          targetRow,
          "Kode Pos"
        ),
        tanggal_dikeluarkan: getSheetValue(
          targetRow,
          "Tanggal Terbit KK"
        ),
  ...
  const akta: AktaResult | null = noAkta
    ? {
        ...
        nama_anak: getSheetValue(
          targetRow,
          "Nama"
        ),
        anak_ke: getSheetValue(
          targetRow,
          "Anak ke"
        ),
        tempat_lahir: getSheetValue(
